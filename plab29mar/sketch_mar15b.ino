...
#define MIN_TURN_DURATION     150 // ms
#define MAX_TURN_DURATION 350
...
void checkLine(bool iChanged, bool changeMomentum)
{
  ...
  if (sensor_values[0] < QTR_THRESHOLD)
  {
    ...
    delay(random(MIN_TURN_DURATION, MAX_TURN_DURATION));
...
void beastModeF()
{
  ...
  if(tick>=updateSpeed)
  {
    ...
    if(r>=30)
    {
      motors.setSpeeds(r*FORWARD_SPEED/100, FORWARD_SPEED);
    }
    else
    {
      motors.setSpeeds(FORWARD_SPEED, (30+r)*FORWARD_SPEED/100);
    }
    updateSpeed=random(200,300);
    tick=0;
  }
  else if(tick>updateSpeed/4)
  {
    motors.setSpeeds(FORWARD_SPEED, FORWARD_SPEED);
  }
  checkLine(true,true);
